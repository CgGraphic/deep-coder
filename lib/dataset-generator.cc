...
experimental::optional<Dataset> generate_dataset(
    ...
    r_for_read.max_length = min((int)max_length,3);
    ...
    enumerate(
        ...
        [&r, &calc_info, &dataset_size, &min_length, &max_length,  &example_per_program, &async_dataset, &time, &cur_dataset_size](const Program &p, const int &i) -> bool {
			...
			auto f = [r, calc_info, &dataset_size, &example_per_program, p, i, &id, &data,&time, &cur_dataset_size]() {
				...
				enumerate(
					r, calc_info,
					[&dataset_size, &example_per_program, &d, &data, &id,&time, &cur_dataset_size](const Program &p,
						const int &i) -> bool {
					// Check program
					//// Unused program
					...
					if (dataset_size == 0) {
						...
						return cur_dataset_size < dataset_size;
					}
				},
					p, i
					);
